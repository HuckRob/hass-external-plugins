...
@Slf4j
@Singleton
public class SandcrabsOverlay extends OverlayPanel {

    private final Client client;
    private final SandcrabsPlugin plugin;
    private final SandcrabsConfiguration config;

    String timeFormat;
    private String infoStatus = "Starting...";

    @Inject
    public SandcrabsOverlay(Client client, SandcrabsPlugin plugin, SandcrabsConfiguration config) {
        super(plugin);
        setPosition(OverlayPosition.TOP_LEFT);
        this.client = client;
        this.plugin = plugin;
        this.config = config;
        getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "sandcrabs overlay"));
